...
* PersonalTags : Améliorer la création de PersonnalTags et ajout d'un servlet qui permet de modifier le tag associer à une personne
* Servlet Personne : faire une doGet des Person qui sont stocké dans la base de données en format Json ( Point ? )

* Servlet Delet:   Ajouter  servlet qui permet aux autor de supprimer les questions de leur choix dans la BDD . la suppression se fait soit avec un id questions, suppression total,par un id author... Dans la servlet elle permet aussi au enseignants de supprimer les examens qui seront créés ( Point ?)- nouvelle fonctionnalité
